...
//					iCount = tranManager(id, "");
					...
					// rs_storge
					strResultmsgString = msg.getMessage() + "。处理时长" + (System.currentTimeMillis()-startTime) + "ms";
					if (flag < 0) {
						// 回滚
				...
				for (int i = 0; i < offerInstAttrList.size(); i++) {
					...
					if (operType.equals("1000")) {
						...
						if (oldOfferInstAttrList.size() == 0) {
							...
							offerInstAttrMap.put("offerInstAttrId", offerInstAttrId);
	...
	// 销售品实例属性
		public int updateOfferInstAtrr(Map itemMap, Map userMap, Message msg,
            ...
            try {
                ...
                List<Map<String, Object>> ordOfferProdInstRelForRole = ordBillDao.selectOrdOfferProdInstRelForRole(offerInstMap);
                if (ordOfferProdInstRelForRole.size() > 0) {
                    Map roleMap = ordOfferProdInstRelForRole.get(0);
                    roleId = roleMap.get("roleId").toString();
                    priceObjectId = roleMap.get("objId").toString();
                }
                ...
                for (Map<String, Object> map : offerInstAttrList) {
                    List<Map<String, Object>> oldOfferInstAttrList = new ArrayList<Map<String, Object>>();
                    ...
                    map.put("action", 1);
                    SynMapContextHolder.addMap("offerInstAttrobjList1", map);
                }*/
	...
	public int insertOfferProdGroup(Map itemMap, Map userMap, Message msg,
		...
		try {
			...
			if (offerProdInstList.size() != 1) {
				msg.setMessage("成员变更未找到商品实例:" + map.get("offerInstId"));
				return -1;
			}
 ...
 */
		...
		try {
			...
			for (Map<String, Object> map : prodInstRelList) {
                ...
                for (Map<String, Object> groupMap : prodInstRelGroupList) {
					...
					long routeId = routeServiceDao.getRouteIdForProdInst(archGrpId, orderItemId, prodInstId, msg);
					if (routeId <= 0 || String.valueOf(routeId) == null) {
						msg.setMessage("获取prod_inst_group路由失败");
						return  - 1;
					}
	...
	public int deleteProdInst(Map itemMap, Map userMap, Message msg)
			...
			if (prodInstList.size() > 0) {
				...
				routeId = routeServiceDao.getRouteIdForProdInst(archGrpId, orderItemId, prodInstId, msg);
				if (routeId == -1) {
					msg.setMessage("删除用户获取routeId失败:" + routeId);
					return -1;
				}
		...
		try {
			...
			Map prodInstMap = new HashMap();
				...
				if (oldProdInstList.size() <= 0) {
					msg.setMessage("产品实例在计费不存在:" + prodInstId);
					return -1;
				}
						...
						if("4070201005".equals(service_offer_id)){
							...
							if (!"3".equals(oldPaymode)) {
								...
								if (prodInstAttrMaps.size() == 1) {
									...
									map.put("action", 2);
									prodinstDao.updateProdInstAttr(map);
									SynMapContextHolder.addMap("prodInstAttrobjList1", map);
					...
					if(("4040600003".equals(service_offer_id)
						...
						if(!"".equals(prodInstMap.get("gsmImsi"))){
							...
							try {
								...
								Long prodInstAccNumId = prodinstDao.getSeq("SEQ_PROD_INST_ACC_NUM_ID"); //统一序列
								prodInstMap.put("prodInstAccNumId", prodInstAccNumId);//需要处理主键prodInstRegionId
								prodInstMap.put("accNumType", "4000");
...
/*  try{
  ...
  if(tax_payerCount==0){
  	...
  	try {
		...
		e.printStackTrace();
		msg.setMessage("取TAX_PAYER.GENERAL_TAXPAYER_FLAG出错");
  		throw e ;
	}
					...
					try {
						...
						if (prodInstAttr1000List.size() == 1) {
							...
							if (attrValue.equals("1")) {
								...
								SynMapContextHolder.addMap("prodInstAttrobjList1", prodInstAttr1000Map);
								return 1;
							}
						}
...
/*				else {
					...
					try {
						...
						if (prodInstAttr1415List.size() == 1) {
							...
							if (attrValue.equals("1")) {
								prodInstAttr1415Map.put("expDate", prodInstMap.get("effDate"));
								prodinstDao.updateProdInstAttrNet(prodInstAttr1415Map);
								SynMapContextHolder.addMap("prodInstAttrobjList1", prodInstAttr1415Map);
    ...
    public int workOrder(Map itemMap, Map userMap, Message msg)
            throws Exception {
        List<Map<String, Object>> ordBillList = ordBillDao.getOrdBill(itemMap);
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        SimpleDateFormat d = new SimpleDateFormat("yyyyMMddHHmmss");

...
public class MqToolActiveUser {
	private static Logger logger = Logger.getLogger(MqToolActiveUser.class);
	...
	private static String topic =propertiesUtil.readProperty("mq.jh.topic");
	...
	static{
        ...
        */
		try {
			properties.setProperty(PropertyKeyConst.ProducerGroupName, propertiesUtil.readProperty("mq.jh.ProducerGroupName"));
			properties.setProperty(PropertyKeyConst.NamesrvAddr, propertiesUtil.readProperty("mq.jh.NamesrvAddr"));
			properties.setProperty(PropertyKeyConst.NamesrvAuthID, propertiesUtil.readProperty("mq.jh.NamesrvAuthID"));

			properties.setProperty(PropertyKeyConst.NamesrvAuthPwd, PasswordUtil.decrypt(propertiesUtil.readProperty("mq.jh.NamesrvAuthPwd"),"bss@2018"));
			properties.setProperty(PropertyKeyConst.ClusterName, propertiesUtil.readProperty("mq.jh.clusterName"));
			properties.setProperty(PropertyKeyConst.TenantID, propertiesUtil.readProperty("mq.jh.TenantID"));
			properties.setProperty(PropertyKeyConst.InstanceName, propertiesUtil.readProperty("mq.jh.InstanceName"));
	...
	public static int send(String topicType, Map msgMap, Message msg) throws Exception {
		try {
			...
			}else if(TopicType.yhjh.equals(topicType)){
				topicName=propertiesUtil.readProperty("mq.jh.topic");
			}
